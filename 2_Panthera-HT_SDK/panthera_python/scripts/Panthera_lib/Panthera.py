...
class Panthera(htr.Robot):  # 继承自htr.Robot
    ...
    def pos_vel_MAXtqe(self, pos, vel, max_tqu, iswait=False, tolerance=0.1, timeout=15.0):
        """关节位置速度最大力矩控制"""
        # 检查关节数量（除了夹爪电机）
    ...
    def gripper_control(self, pos, vel, max_tqu):
        """夹爪控制（位置速度最大力矩模式）"""
    ...
    def gripper_open(self, vel=0.5, max_tqu=0.5):
        """打开夹爪"""
        self.gripper_control(0.8, vel, max_tqu)
...
#######################运动学和动力学函数#######################
    ...
    def inverse_kinematics(self, target_position, target_rotation=None, init_q=None, max_iter=1000, eps=1e-4):
        """使用pinocchio计算逆运动学"""
        if self.model is None:
            return None
    ...
    def quintic_interpolation(self, start_pos, end_pos, duration, current_time):
        """五次多项式插值轨迹生成（速度、加速度连续），返回np.ndarray"""
        # 转换为numpy数组
        ...
        # 五次多项式系数 (位置)
        a0 = 1 - 10*t3 + 15*t4 - 6*t5
        a1 = 10*t3 - 15*t4 + 6*t5

        # 一阶导数系数 (速度)
        da0 = -30*t2 + 60*t3 - 30*t4
        da1 = 30*t2 - 60*t3 + 30*t4

        # 二阶导数系数 (加速度)
        dda0 = -60*t + 180*t2 - 120*t3
        dda1 = 60*t - 180*t2 + 120*t3
