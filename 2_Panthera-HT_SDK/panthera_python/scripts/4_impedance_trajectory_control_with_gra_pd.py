...
from scripts.Panthera_lib.Panthera import Panthera  # 从Panthera_lib子目录导入Panthera类
...
if __name__ == "__main__":
    try:
        robot = Panthera()
        # 阻抗控制参数
        K = np.array([2.0, 4.0, 8.0, 1.0, 1.0, 1.0])
        B = np.array([0.5, 1.0, 1.40, 0.1, 0.1, 0.1])
        ...
        while(1):
            ...
            # 发送控制命令
            robot.pos_vel_tqe_kp_kd(zero_pos, zero_vel, tor, zero_kp, zero_kd)
            # robot.pos_vel_tqe_kp_kd(zero_pos, zero_vel, [0.0]*6, zero_kp, zero_kd, iswait=False)
            print(tor)
